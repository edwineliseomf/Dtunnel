...
list.register(new Observer('append', async item => {
    ...
    try {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess('Configuración creada correctamente!');
            renderApp();
            return;
        }
        ...
        showToastError('No se pudo crear la configuración!');
        list.remove(item.id);
        categories.remove(item.id);
    }
...
list.register(new Observer('update', async item => {
    ...
    try {
        ...
        showToastError('No se pudo actualizar la configuración!');
    } finally {
        renderApp();
    }
}));
...
list.register(new Observer('remove', async id => {
    ...
    try {
        ...
        showToastError('No se pudo quitar la configuración!');
    } finally {
        renderApp();
    }
}));
