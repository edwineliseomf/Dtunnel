...
const main = async () => {
    ...
    categoryList.register(new Observer('add', async category => {
        ...
        if (response.status == 201) {
            ...
            showToastSuccess(`Categoria ${category.name} creada con éxito!`)
            render()
            return
        }
    ...
    categoryList.register(new Observer('remove', async category => {
        ...
        try {
            ...
            if (response.status == 204) {
                showToastSuccess(`Categoria ${category.name} eliminada con éxito!`)
                return
            }
            ...
            showToastError(`Error al eliminar categoría ${category.name}!`)
        }
    ...
    categoryList.register(new Observer('update', async category => {
        ...
        if (data.status == 200) {
            showToastSuccess(`Categoria ${category.name} actualizada con éxito!`)
            return;
        }

        showToastError(`Error al actualizar categoría ${category.name}!`)
    }))
